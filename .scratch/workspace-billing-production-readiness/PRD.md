...
- [ ] The live webhook endpoint is configured as an account-scoped HTTPS endpoint for `https://app.example.com/v1/billing/stripe/webhook`. It must not be an organization-wide endpoint, must not listen to connected-account events, and must not subscribe to all events.
